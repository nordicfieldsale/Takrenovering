...
// CORS gäller ENBART API:t. Tidigare låg kontrollen globalt, vilket gjorde att
// den även träffade CSS- och JS-filerna (Vite märker dem med crossorigin, så
// webbläsaren skickar en Origin-header även på samma domän). Resultatet blev
// 403 på allt utom bilderna, och därmed en helvit sida i produktion.
const corsCheck = cors({
  origin(origin, callback) {
    if (!origin) return callback(null, true);
    if (config.corsOrigins.includes(origin)) return callback(null, true);
    // Anrop från appens egen adress ska alltid släppas igenom.
    if (config.appUrl && origin === config.appUrl) return callback(null, true);
...
if (fs.existsSync(webRoot)) {
  ...
  // Syns tydligt i stället för en tom vit sida om bygget av webbappen
  // skulle ha misslyckats.
  app.use((_req, res) =>
    res
      .status(503)
      .type('text/plain; charset=utf-8')
      .send('Webbappen är inte byggd. Kontrollera att "npm run build" kördes i web/.')
  );
}
